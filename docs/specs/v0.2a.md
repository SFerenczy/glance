...
- Tab or Enter accepts selected suggestion
...
> As a user, I want to copy SQL and results to my clipboard.

**Acceptance Criteria:**

- `y` key copies last executed SQL to clipboard
- Text selection with mouse supported
- Selected text can be copied with Ctrl+C (when not in input mode)
- Visual feedback confirms copy action

### US-5: Vim-Style Navigation

> As a user familiar with vim, I want to navigate using familiar keybindings.

**Acceptance Criteria:**

- `j`/`k` scroll chat when not in input mode
- `g` jumps to top of chat
- `G` jumps to bottom of chat
- `Ctrl+↑/↓` scrolls one line at a time
- Navigation keys only active when input is not focused
...
| Tab    | Accept selected, keep palette  |
| Enter  | Accept selected, close palette |
...
1. **Vim mode default**: Should vim-style navigation be opt-in or opt-out?
   - _Proposed_: Opt-in via config, with first-run prompt
