...
### Phase 5 - LLM Improvements (PARTIAL - Backend Complete)
- ✅ Multi-tag filtering: src/persistence/saved_queries.rs:53,233-242, src/llm/service.rs:319-326 (AND semantics)
- ✅ Redacted connection context: src/llm/service.rs:345-365 (label + database only, no host/user/password)
- ⏸️ Masked input for /llm key: Requires TUI changes

### Phase 6 - Persistence Robustness (PARTIAL - Core Complete)
- ✅ Schema version mismatch check: src/persistence/migrations.rs:15-21 (fails with guidance if DB newer than code)
- ✅ DB recovery tracking: src/persistence/mod.rs:153,310 (recovered field and was_recovered() method)
- ⏸️ with_retry usage: Infrastructure exists at src/persistence/mod.rs:102 but not used at call sites
- ⏸️ DB recovery toast: Requires TUI changes (infrastructure ready via was_recovered())
- ⏸️ Secret storage badge: Requires TUI changes (infrastructure ready via secret_storage_status())
